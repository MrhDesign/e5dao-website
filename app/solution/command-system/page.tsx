      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-2 gap-2.5">
          {mobileCommandData?.features?.content?.split('\n').map((feature: string, index: number) => {
            ...
            return (
              <div key={index} className="bg-fill-two rounded-lg p-2.5 hover:bg-fill-three transition-colors duration-200">
                <h3 className="text-2xl font-medium text-text-brand mb-3">
                  {title}
                </h3>
                <p className="text-text-display text-xl leading-relaxed">
                  {description}
                </p>
              </div>
            );
